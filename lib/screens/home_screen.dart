  ...
  Future<void> _handleGetStarted() async {
    ...
    setState(() {
      ...
      _statusMessage = _readiness == gemma.GemmaReadiness.sharedFileAvailable
          ? 'Model installation from shared file...'
          : 'Downloading the AI model (2,4 Go)...';
    });
    ...
    try {
      await gemma.setupGemmaModel(
        onProgress: (p) {
          if (mounted) {
            setState(() {
              _downloadProgress = p;
              _statusMessage = 'Downloading the AI model... $p%';
            });
          }
        },
      );
      ...
      if (mounted) {
        setState(() {
          _statusMessage =
              'Preparing the model (may take a few minutes on the first launch)...';
        });
      }
